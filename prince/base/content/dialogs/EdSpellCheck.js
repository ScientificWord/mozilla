...
function Startup()
{
  ...
  var editor = msiGetEditor(window.arguments[3]);
  if (!editor)
  {
    window.close();
    return;
  ...
  try {
    ...
    dump("curLang is "+curLang+"\n");
  } catch(ex) {
    curLang = "";
    dump("*** GetCurrentDictionary failed\n");
  }
  dump("curLang is "+curLang+"\n");
...
{
  window.openDialog("chrome://prince/content/EdDictionary.xul", "editdictionary", "chrome,close,titlebar,dependent,resizable", "", gMisspelledWord);
}
...
function Recheck()
{
  ...
  try {
    ...
    gSpellChecker.InitSpellChecker(msiGetEditor(window.arguments[3]), false);
...
function ExitSpellChecker()
{
  if (gSpellChecker)
  {
    try
    {
      ...
      if (inlineSpellChecker.enableRealTimeSpell)
        inlineSpellChecker.checkDocument(window.opener.content.document);
