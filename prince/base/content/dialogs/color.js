...
{ 
  ...
  if (node)
  {
    initialColor = node.getAttribute("color");
...
function onAccept()
{
  ...
  if (node)
  {
    node.setAttribute("color", gColorObj.TextColor);
    node.setAttribute("style", "color: "+gColorObj.TextColor+";");
  }
  else 
  { 
    theWindow.msiRequirePackage(editorElement, "xcolor", null);
    theWindow.msiEditorSetTextProperty(editorElement, "fontcolor", "color", gColorObj.TextColor);
