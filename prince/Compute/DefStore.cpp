...
DefStore::DefStore(DefStore * parent_store, U32 client_handle) :
  parent_ds(parent_store), client_ID(client_handle), def_list(NULL), prefs_list(NULL)
{
}
...
void DefStore::PushDefInfo(U32 engine_ID,
                           const char *def_name, U32 def_type,
                           ...
                           const char *ASCII_src, const U16 * WIDE_src)
{
  if (def_name && *def_name) {
    ...
    DefInfo *new_definfo = TCI_NEW(DefInfo());
    ...
    size_t zln = strlen(def_name);
    char *tmp = TCI_NEW(char[zln + 1]);
    strcpy(tmp, def_name);
    new_definfo->canonical_name = tmp;
    if (markup && *markup) {
      size_t zln = strlen(markup);
      char *tmp = TCI_NEW(char[zln + 1]);
      strcpy(tmp, markup);
      new_definfo->src_markup = tmp;
    } else
      new_definfo->src_markup = NULL;
    if (arg_list && *arg_list) {
      size_t zln = strlen(arg_list);
      char *tmp = TCI_NEW(char[zln + 1]);
      strcpy(tmp, arg_list);
      new_definfo->arg_list = tmp;
    } else
      new_definfo->arg_list = NULL;
    if (ASCII_src && *ASCII_src) {
      size_t zln = strlen(ASCII_src);
      char *tmp = TCI_NEW(char[zln + 1]);
      strcpy(tmp, ASCII_src);
      new_definfo->ASCII_src = tmp;
    } else
      new_definfo->ASCII_src = NULL;
...
DefInfo *DefStore::GetLocalDefInfo(U32 engine_ID, const char *targ_canon_nom)
{
  if (targ_canon_nom && *targ_canon_nom) {
    ...
    while (dl_rover) {
      if (dl_rover->engine_ID == engine_ID
          && !strcmp(dl_rover->canonical_name, targ_canon_nom))
        return dl_rover;
...
DefInfo *DefStore::DeleteDefInfo(U32 engine_ID, const char *targ_canon_nom)
{
  ...
  while (dl_rover) {
    ...
    dl_rover = dl_rover->next;
    if (curr->engine_ID == engine_ID
        && !strcmp(curr->canonical_name, targ_canon_nom)) {
