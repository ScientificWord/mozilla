...
SWPPlotmatrix := Dom::Matrix(  ):

//The idea here is as follows. We're asked to "plot" either an expression or a list of expressions.
//We first categorize the expressions and any variables explicitly involved in them, hoping to 
//  represent them as either functions or assignment expressions. In the case of a functional 
//  expressions, this indicates an as-yet unnamed variable to carry the functional value; we 
//  represent this by entering a 0 in the list of dependent vars. In the case of an assignment 
//  expression, this indicates a named variable carrying the functional value; we enter the named 
//  variable in the list of dependent vars.
//Now we want to check that a variable name and expression are determined for each result variable.
//  If we have entered a 0 for an expression, we may assume that the intended variable name is not
//  contained in the expression, so we do not attempt to pick up a name from the independent variable
//  list in this case. Instead, we begin by matching the last unnamed (needed) variable to the last
//  previously unused variable in the "proposed" result variable list, and proceed backwards towards
//  the first unnamed variable. If we now find that insufficiently many result variables have been
//  specified, the first assumption is that the plot is intended to be more a function graph than
//  a function image, and we fill in the missing dimensions from the independent variables. If we're
//  still missing any, we take one of the as-yet unused proposed return variable names (the first
//  available), and add it in its correct place, with the defining expression for it being itself.

swPreparePlot30 := proc(proposedParams,proposedOutVars,quotedList,dim,inType)
local theList, newList, retType, retParams, retDepVars, retList,
      i, j, defList, tempIndets, tempDepVars, tempRetTypes, tempList, 
      bNeedToReSort, bIsReallyConstant,
      tempDefList, tempRetList, bSomethingWrong, retTypeSet, foundIndex,
      proposedParamsUsed, proposedDepVarsUsed, foundParamsLeft, foundDepVarsLeft,
      doAnItem, collateVars, sortVarProc, adjustMaxParams, extraArgs, subsTable, 
      ourNames, SWmakeSubsTable,
      SWcheckForProcs, SWreplaceProcNames, matrixSize, nParamsNeeded, nVarsNeeded, 
      nMaxParams, bAnimated,
      animParam, bOnlyAnimatedParam, bParametric, extraFlags,
	  killOneUnit, SWPisConstant;
begin

sortVarProc := proc(a,b) begin bool(expr2text(a) < expr2text(b)) end_proc:

killOneUnit := proc(theItem)
  local units;
begin
  units := tciindets(theItem);
  units := units minus map(units,tciremoveunits);
  if (nops(units) = 1) then
    subs(theItem,op(units,1)=1);
  else
    theItem;
  end_if;
end;

SWPisConstant := proc(expr)
  //evaluate deeply to determine is expr is a constant expression
  save LEVEL;
begin
  LEVEL := 20; 
  if type(expr) = DOM_STRING then
    SWPisConstant(eval(text2expr(expr)));
  else 
    if nops(expr) > 1 then
      if contains(map({op(expr)},SWPisConstant),FALSE) then
        FALSE;
      else
        TRUE;
      end_if:
    else
      testtype(eval(expr), Type::Constant); 
    end_if:
  end_if:
end_proc:
...
newSWmakeSubsTable := proc(anExpr,theIndets)
local theTable,anIdent,aVal,i,nValues,returnExpr,newIdent,theParams,evalExpr;
option escape;
begin
  theTable := {}:
  evalExpr := text2expr(anExpr):
  if traperror(eval(evalExpr)) > 0 or type(eval(evalExpr)) = DOM_PROC then
    newIdent := genident("subExpr"):
    theTable := theTable union {[newIdent, anExpr, op(evalExpr,1)]}:
//      theTable := theTable union {[newIdent,expr2text(op(anExpr,6)),op(anExpr,1)]}:
    returnExpr := newIdent:
  else
    returnExpr := evalExpr:
    for i from nops(returnExpr) downto 0 do
      if (i>0) or (contains({"function"},type(returnExpr))) then
        if type(returnExpr) = piecewise then
          anIdent := extop(returnExpr,i):           
        else
          anIdent := op(returnExpr,i):
        end_if:
        if type(anIdent) = DOM_IDENT then
          if contains({"_fconcat",DOM_PROC,DOM_EXEC},type(eval(anIdent))) then
            newIdent := genident("subExpr"):
            theParams := []:
            if type(eval(anIdent)) = DOM_PROC then
              theParams := op(eval(anIdent),1):
            elif type(eval(anIdent)) = "_fconcat" then
              theParams := op(eval(anIdent),[nops(eval(anIdent)),1]):
            end_if:
            theTable := theTable union {[newIdent,expr2text(anIdent),theParams]}:
            if type(evalExpr) = piecewise then
              returnExpr := extsubsop(returnExpr,i=newIdent):
            else
              returnExpr := subsop(returnExpr,i=newIdent):
            end_if:
          end_if:
        elif not contains({DOM_IDENT,DOM_DOMAIN},type(anIdent)) then
          if not SWPisConstant(anIdent) then
            theParams :=  newSWmakeSubsTable(expr2text(anIdent)):
            theTable := theTable union op(theParams,2):
            if type(returnExpr) = piecewise then
              returnExpr := extsubsop(returnExpr,i=op(theParams,1)):
            else
              returnExpr := subsop(returnExpr,i=op(theParams,1)):
            end_if:
          end_if:
        end_if:
      end_if:
...
SWmakeSubsTable := proc(anExpr,theIndets)
local theTable,anIdent,aVal,i,nValues,returnExpr,newIdent,theParams;
option escape;
begin
  theTable := {}:
  if type(anExpr) = DOM_PROC then
    newIdent := genident("subExpr"):
      theTable := theTable union {[newIdent,expr2text(op(anExpr,6)),op(anExpr,1)]}:
    returnExpr := newIdent:
  else
    returnExpr := anExpr:
    for i from nops(anExpr) downto 0 do
      if (i>0) or (contains({"function"},type(anExpr))) then
        if type(anExpr) = piecewise then
          anIdent := extop(anExpr,i):           
        else
          anIdent := op(anExpr,i):
        end_if:
        if type(anIdent) = DOM_IDENT then
          ...
          if not testtype(anIdent,Type::Constant) then
            theParams :=  SWmakeSubsTable(anIdent):
            theTable := theTable union op(theParams,2):
            if type(anExpr) = piecewise then
              returnExpr := extsubsop(returnExpr,i=op(theParams,1)):
            else
...
SWcheckForProcs := proc(theExpr,currProcVarList)
local localProcVarList,localExprList,localProcList,splitList,myExpr,i,nIndex;
begin
  localProcList := []:
  nIndex := contains(ourNames,theExpr):
  if nIndex <> 0 then
    localProcList := [nIndex]:
  elif testtype(theExpr,"_fconcat") then
    myExpr := {op(theExpr,nops(theExpr))}:
    nIndex := contains(ourNames,myExpr):
    if nIndex <> 0 then
      localProcList := [nIndex]:
    end_if:
  elif domtype(theExpr) <> DOM_IDENT then
    localExprList := op(split([op(theExpr)],hastype,DOM_IDENT),1):
    for i from nops(localExprList) downto 1 do
      myExpr := op(localExprList,i):
      currProcVarList := SWcheckForProcs(myExpr,currProcVarList):
    end_for:
  end_if:
  for i in localProcList do
    localProcVarList := op(subsTable,[i,3]):
    if localProcVarList <> NIL then
      if domtype(localProcVarList) = DOM_IDENT then
        localProcVarList := [localProcVarList]:
      end_if:
      //SLS Make sure function gets all the variables.
      //SLS I'm not sure the following is correct in all situations -- empirically derived code
      if (dim = 3) and (op(subsTable,[i,4]) <> NIL) then
        localProcVarList := append(localProcVarList,op(subsTable,[i,4]));
      end_if:
    end_if:
    if nops(localProcVarList) > nops(currProcVarList) then
      currProcVarList := localProcVarList:
    end_if:
  end_for:
  currProcVarList;
end_proc;

SWreplaceProcNames := proc(anExpr)
local i,foundAt,theName,returnExpr,nIndex,nLength,bFoundOne;
begin
  returnExpr := expr2text(anExpr):
  bFoundOne := FALSE:
  for nIndex from 1 to nops(ourNames) do
    theName := expr2text(ourNames[nIndex]):
    nLength := length(theName):
    foundAt := stringlib::contains(returnExpr,theName,IndexList):
    for i from 1 to nops(foundAt) do
      bFoundOne := TRUE:
      returnExpr := stringlib::subsop(
               returnExpr,op(foundAt,i)..op(foundAt,i)+nLength-1 = op(subsTable,[nIndex,2])):
    end_for:
  end_for:
  if (bFoundOne) then
    returnExpr := "hold(" . returnExpr . ")":
  end_if:
  returnExpr;
end_proc;

adjustMaxParams := proc(theDim, theType, bIsAnimated)
local maxNumParams;
begin
  maxNumParams := 4:  //just an initialization - this would be the absolute max in any case
  case retType
    of "inequality" do
    of "equality" do
    of "vectorField" do
    ...
    break:
    otherwise
      maxNumParams := theDim - 1:
    break:
  end_case:
  if bIsAnimated then
    maxNumParams := maxNumParams + 1:
  end_if:
  maxNumParams;
end_proc:

doAnItem := proc(theItem)
  local ourIndets, ourParamVars, ourDepVars, ourDefList, ourReturnType, ourRetList, newItem, i;
  //Additionally, we'll refer to the "global" (in the enclosing proc) variables:
  //    tempDepVars, tempIndets, defList
begin
  ourDefList := []:
  ourDepVars := []:
  ourParamVars := []:
  ourReturnType := "":
  ourRetList := []:
  newItem := killOneUnit(theItem);
  ourIndets := [op(select(numeric::indets(newItem), not testtype, Type::Constant))]:
  for i from nops(ourIndets) downto 1 do
    if contains(ourNames,ourIndets[i]) <> 0 then
      delete(ourIndets[i]):
    end_if:
  end_for:
  if testtype(newItem, Type::Relation) then
    if type(newItem) = "_equal" then
      if (inType <> equality) and (contains(ourIndets, op(newItem,1)) and 
                              not has(op(newItem,2),op(newItem,1))) then
        ourReturnType := "functionImage":
        ourDepVars := [op(newItem,1)]:
        ourDefList := append(ourDefList,[op(newItem,1),op(newItem,2)]):
      elif (inType <> equality) and (contains(ourIndets, op(newItem,2)) and 
                                not has(op(newItem,1),op(newItem,2))) then
        ourReturnType := "functionImage":
        ourDepVars := [op(newItem,2)]:
        ourDefList := append(ourDefList,[op(newItem,2),op(newItem,1)]):
      else 
        ourReturnType := "equality":
      end_if:
      ourRetList := [op(newItem,1) - op(newItem,2)]:
    else
      ourReturnType := "inequality": 
      ourRetList := [newItem]:
    end_if:
  elif (nops(ourIndets) = 0) or 
        contains({DOM_IDENT,DOM_EXPR,DOM_PROC,piecewise}, domtype(newItem)) then
    ourReturnType := "functionImage":
    ourDepVars := append(ourDepVars,0):
    ourDefList := append(ourDefList,[0,newItem]):
  end_if:
  for i in ourIndets do
    if contains(ourDepVars,i) = 0 then
      ourParamVars := append(ourParamVars, i):
    end_if:
  end_for:
  ourParamVars := sort(ourParamVars, sortVarProc):
  [ourReturnType, ourParamVars, ourDepVars, ourDefList, ourRetList];
end_proc:

// goes through list of expressions within one item (e.g. parametric thing), 
// for each expr in the list,
// record what it says are the independent vars. CollateVars tries to see if 
// you have the right number,
// fill in from default, etc... (ugly "fix-up-the-vars-you-found" function).
collateVars := proc(locParams,locDefList,nParamsNeeded,nVarsNeeded,nMaxParams)
local i, j, k, m, dummyFunc, foundIndex, returnParams, returnDefList, returnDepVars, 
               nParams, tempDefList, 
               desiredOrder, thisExpr, nAnimParamSet, isReallyParametric;
begin
  returnParams := []:
  returnDefList := []:
  isReallyParametric := TRUE:  //will be fixed up below when filling returnDefList
  nAnimParamSet := 0:
  if animParam <> 0 then
    nAnimParamSet := 1:
    if contains(locParams, animParam) = 0 then
      if nops(locParams) < nMaxParams then
        locParams := append(locParams, animParam):
      else  //should we do this? Idea is that if proposed animation param doesn't appear 
            //in the expression and there isn't room for it, it must not be correct...
        animParam := 0:
        nAnimParamSet := 0:
      end_if:
    end_if:
...
//1: Take params found and record matches against proposedParams; then match dependent 
// variable names found
//against proposed variable names. Finally, when a found parameter matches a dependent 
// variable name, IF there is only
//one expression found (so that the expression is not parametric), go ahead and mark that 
// variable name as matched.

for i from 1 to nops(locDefList) do
  foundIndex := contains(proposedOutVars, locDefList[i][1]):
  if foundIndex = 0 then
    foundDepVarsLeft := append(foundDepVarsLeft,i):
  else
    proposedDepVarsUsed := append(proposedDepVarsUsed,foundIndex):
  end_if:
end_for:

//2: At this point we should have recorded everything found that matched stuff passed in.
//reorder proposed variable names, for the various coordinate systems
  desiredOrder := []:
  if contains(extraArgs,cylindrical) <> 0 then
    desiredOrder := [2,3,1]:
  elif contains(extraArgs,spherical) <> 0 then
    desiredOrder := [2,3,1]:
  elif contains(extraArgs,polar) <> 0 then
    desiredOrder := [2,1]:
  else
    desiredOrder := [i $ i=1..dim]:
  end_if:
//  if contains(extraArgs,animated) <> 0 then
//    desiredOrder := [dim+1] . desiredOrder:
//  end_if:

  //Now fill in unnamed found Vars, from the end of the list to the beginning (showing 
  // preference
//  for specific variables (normally the last, but not for cylindrical/spherical!) 
// as image of a function).
  for k from nops(desiredOrder) downto 1 do
    i := desiredOrder[k]:
    foundIndex := contains(foundDepVarsLeft,i):
    if i<=nops(locDefList) and (locDefList[i][1] = 0) then //this one not already matched
      for m from nops(desiredOrder) downto 1 do
        j := desiredOrder[m]:
        if (j<=nops(proposedOutVars) and (contains(proposedDepVarsUsed,j) = 0)) then
         //We haven't yet assigned this axis name to an output variable. However, 
         // if it appears in the 
         // input expression as anything other than itself,
         //we shouldn't assign it to this one!
          if (contains(locParams,proposedOutVars[j]) = 0 or 
                                   not has(op(locDefList,[i,2]),proposedOutVars[j]) 
                 or op(locDefList,[i,2]) = proposedOutVars[j]) or nops(locDefList) = dim then
            locDefList[i][1] := proposedOutVars[j]:
            proposedDepVarsUsed := append(proposedDepVarsUsed,j):
            delete foundDepVarsLeft[foundIndex]:
            break:
          end_if:
        end_if:
      end_for:
  ...
  returnDepVars := []:
  for i from 1 to nops(locDefList) do
    returnDepVars := append(returnDepVars,locDefList[i][1]):
  end_for:  
  //Note that we'll repeat this process below, after filling more of "locDefList". This is 
  // preliminary in order to set the animation variable.
  //First nail down the animation parameter - if we're animated, it should probably get 
  // assigned to one of the found variables (unless passed in).
  if bAnimated and animParam = 0 then
    for i from nops(locParams) downto 1 do
      if contains( returnDepVars, locParams[i] ) = 0 and 
         contains( proposedOutVars, locParams[i] ) = 0 then
        animParam := locParams[i]:
        break:
      end_if:
    end_for:
    if animParam = 0 and nops(proposedParams) > 0 then
      animParam := proposedParams[nops(proposedParams)]:
    end_if:
  end_if:
  if animParam <> 0 then
    nAnimParamSet := 1:
    foundIndex := contains(locParams, animParam):
    if foundIndex <> 0 then
      delete locParams[foundIndex]:
    end_if:
    foundIndex := contains(proposedParams, animParam):
    if foundIndex <> 0 then
      delete proposedParams[foundIndex]:
    end_if:
  end_if:

  nParams := nops(locParams):
  //Find last unused proposed variable first:
  for k from nops(desiredOrder) downto 1 do
    i := desiredOrder[k]:
    if (nops(locDefList) + nParams + nAnimParamSet) >= nVarsNeeded then
      break;
    end_if:
    if (i<=nops(proposedOutVars)) and (contains(proposedDepVarsUsed,i) = 0) and 
        not has(locDefList,proposedOutVars[i]) and 
        contains(locParams,proposedOutVars[i]) = 0 then
      if not bAnimated or (proposedOutVars[i] <> animParam and 
         (animParam <> 0 or proposedOutVars[i] <> proposedParams[nops(proposedParams)])) then
        locDefList := [[proposedOutVars[i],proposedOutVars[i]]] . locDefList:
        locParams := append(locParams,proposedOutVars[i]):
        proposedDepVarsUsed := append(proposedDepVarsUsed,i):
      end_if:
    end_if:
  end_for:
  
  for i from 1 to nops(locParams) do
    foundIndex := contains(proposedParams,op(locParams,i)):
    if foundIndex = 0 and contains(foundParamsLeft, op(locParams,i)) = 0 then
      if op(locParams,i) <> animParam then
        foundParamsLeft := append(foundParamsLeft, op(locParams,i)):
      end_if:
    else
      proposedParamsUsed := append(proposedParamsUsed, foundIndex):
    end_if:
  end_for:

  //At this point, locDefList should contain no more 0's. If it does, we're in trouble!
  //Fix up the "params" next:
  if (nops(foundParamsLeft) > 0) then
    foundParamsLeft := sort(foundParamsLeft, sortVarProc):
  end_if:
  for i from 1 to max(nops(locParams),nops(proposedParams)) do
    if (nops(foundParamsLeft) = 0 and nops(locParams) <= nops(returnParams)) then
      break:
    end_if:
    if contains(proposedParamsUsed,i) <> 0 then
      returnParams := append(returnParams,proposedParams[i]):
    elif nops(foundParamsLeft) > 0 then
      returnParams := append(returnParams,foundParamsLeft[1]):
      delete(foundParamsLeft[1]):
    end_if:
  end_for:
  if animParam <> 0 then
    returnParams := append(returnParams, animParam):
  end_if:

  nParamsNeeded := max(nParamsNeeded,nops(locParams)):

  //Finally, we check whether we have enough return variables. If size is less than 
  // dimension, it
  //  means not enough expressions were specified to fill the dimensions; we 
  // proceed through the
  //  returnParams trying to fill it up first, then get any unused proposedDepVars.
  returnDepVars := []:
  for i from 1 to nops(locDefList) do
    returnDepVars := append(returnDepVars,locDefList[i][1]):
  end_for:
  while (nops(locDefList) < dim) do
    if nops(returnParams) = 0 then
      error("Can't find enough variable names for this dimension!"):
      break:
    end_if:
    isReallyParametric := FALSE:
    for i from nops(returnParams) downto 1 do
      if contains(returnDepVars,returnParams[i]) = 0 then
        if not(returnParams[i] = animParam) then
          locDefList := [[returnParams[i],returnParams[i]]] . locDefList:
          returnDepVars := [returnParams[i]] . returnDepVars:
          foundIndex := contains(proposedOutVars,returnParams[i]):
          if foundIndex <> 0 then
            proposedDepVarsUsed := append(proposedDepVarsUsed,foundIndex):
          end_if:
          break:  //out of "for"
  ...
  for i from 1 to dim do
    j := 1:
    if (nops(locDefList) = 0) then
      error("Can't find enough variable names for this dimension!"):
    end_if:
    foundIndex :=  contains(proposedDepVarsUsed,i):
    if foundIndex <> 0 then
      j := contains(returnDepVars,proposedOutVars[i]):
    end_if:
    if j = 0 then
      j := 1:
    end_if:
    localProcVarList := SWcheckForProcs(op(locDefList,[j,2]),[]):
    if localProcVarList = NIL then
      thisExpr := (op(locDefList,[j,2]))(op(returnParams)):
      locDefList[j][2] := thisExpr:
    elif (nops(localProcVarList) > 0) and (nops(localProcVarList) <= dim) then
      for k from 1 to nops(localProcVarList) do
        if k <= nops(returnParams) then
          localProcVarList[k] := returnParams[k]:
        elif k <= nops(proposedParams) then
          for ii from 1 to nops(proposedParams) do 
            if contains(proposedParamsUsed,ii)=0 then
              localProcVarList[k] := proposedParams[ii]:
              returnParams := append(returnParams,proposedParams[ii]):
              proposedParamsUsed := append(proposedParamsUsed,ii):
              break:
            end_if:
          end_for:
        else
          localProcVarList[k] := proposedOutVars[k]:
        end_if:
      end_for:
      thisExpr := (op(locDefList,[j,2]))(op(localProcVarList)):
      locDefList[j][2] := thisExpr:
    end_if:
    returnDefList := append(returnDefList,locDefList[j]):
    delete(locDefList[j]):
    delete(returnDepVars[j]):  
  end_for:
  //Now fill in the returnParams with leftover proposed ones if needed.
  for i from 1 to nops(proposedParams) do
    if nops(returnParams) >= nParamsNeeded then
      break:
    end_if:
    if contains(proposedParamsUsed,i) = 0 then
      returnParams := [returnParams[ii] $ ii=1..i-1] . [proposedParams[i]] . [returnParams[ii] $ 
                                                                   ii=i..nops(returnParams)]:
    end_if:
  end_for:
  //Finally, resort returnParams to put animParam last:
  if animParam <> 0 then
    foundIndex := contains(returnParams, animParam):
    if foundIndex < nops(returnParams) then
      returnParams := [returnParams[ii] $ ii=1..foundIndex-1] . [returnParams[ii] $
                                          ii = foundIndex+1..nops(returnParams)] . [animParam]:
    end_if:
  end_if:
  [returnParams,returnDefList,isReallyParametric];
end_proc:

if type(quotedList) = DOM_STRING then
  if not (SWPContainsProc(quotedList) = TRUE)then
    LEVEL := 100:
    quotedList := expr2text(eval(text2expr(quotedList))):
	delete LEVEL:
  end_if:
  newList := newSWmakeSubsTable(quotedList):
  theList := eval(text2expr(quotedList)):
  delete LEVEL:
else
  theList := quotedList:
  newList := SWmakeSubsTable(theList):
end_if:
subsTable := op(newList,2):
ourNames := [op(subsTable,[i,1]) $ i=1..nops(subsTable)]:
newList := eval(op(newList,1)):
bAnimated := FALSE:
animParam := 0:
bOnlyAnimatedParam := FALSE:
extraFlags := []:
bParametric := FALSE:
extraArgs := []:
if args(0) > 5 then
  extraArgs := [args(i) $ i=6..args(0)]:
  for i from 1 to nops(extraArgs) do
    if extraArgs[i] = animated then
      bAnimated := TRUE:
    elif type(extraArgs[i]) = "_equal" and op(extraArgs[i],1) = animationParam then
      animParam := op(extraArgs[i],2):
      bAnimated := TRUE:
    end_if:
  end_for:
end_if:
...
retTypeSet := {}:
matrixSize := []:
nParamsNeeded := 0:  //provisional value
nVarsNeeded := dim:
nMaxParams := dim:

if (testtype(newList,Dom::Matrix) and not testtype(newList,DOM_ARRAY)) then
  matrixSize := SWPmatrix::matdim(newList):
  if matrixSize[1] = 1 then
    newList := array(1..matrixSize[2],[op(newList)]):
  else
    newList := array(1..matrixSize[1],1..matrixSize[2],[[newList[i,j] $ 
                                 j=1..matrixSize[2]] $ i=1..matrixSize[1]]):
  end_if:
end_if:
if (testtype(newList,DOM_LIST) and not testtype(newList,DOM_ARRAY)) then
  if testtype(op(newList,1),DOM_LIST) then
    newList := array(1..nops(newList),1..nops(op(newList,1)),newList):
  else
    newList := array(1..nops(newList),newList):
  end_if:
end_if:

//set up "nParamsNeeded"
case inType
  of explicitList do
    //nothing - nParamsNeeded = 0 is okay
  break:
  of vectorField do
    nParamsNeeded := dim:
  break:
  otherwise
    nParamsNeeded := 1:
    //and we leave nMaxParams set at "dim"
  break:
end_case:
if bAnimated then
  nParamsNeeded := nParamsNeeded + 1:
  nVarsNeeded := nVarsNeeded + 1:
  nMaxParams := nMaxParams + 1:
end_if:

if testtype(newList, DOM_ARRAY) then
  tempIndets := select(numeric::indets(newList), not SWPisConstant): 
  bIsReallyConstant := FALSE:
  if bAnimated and nops(tempIndets) = 1 then
    if animParam = 0 then
      if tempIndets[1] = proposedParams[nops(proposedParams)] and 
         contains(proposedOutVars,tempIndets[1]) = 0 then
        bOnlyAnimatedParam := TRUE:
      end_if:
    elif tempIndets[1] = animParam then
      bOnlyAnimatedParam := TRUE:
    end_if:
  end_if:
  if nops(tempIndets) = 0 or bOnlyAnimatedParam then
    if SWPisConstant(newList) or bOnlyAnimatedParam then
      case inType
        of vectorField do
        of curviLinear do
          bIsReallyConstant := FALSE:
        break:
        otherwise
          bIsReallyConstant := TRUE:
        break:
      end_case:
    end_if:
  end_if:
//  if (op(newList,[0,1]) = 1 and op(newList,[0,2,2]) = dim) then //just a list?
  if (op(newList,[0,1]) = 1) then //just a list?
    if bIsReallyConstant=TRUE and op(newList,[0,2,2]) = dim then
      retType := "explicitList":
      retList := [[op(newList,j) $ j = 1..dim]];
      retList := float(retList):
      retDepVars := proposedOutVars:
      if bAnimated=TRUE then
        retParams := [op(tempIndets)]:
      end_if:
    else
      for i from 1 to op(newList,[0,2,2]) do
        tempList := append(tempList,newList[i]):
      end_for:
    end_if:

  elif op(newList,[0,1]) = 2 then
    if bIsReallyConstant=TRUE and op(newList,[0,3,2]) = dim then
      retType := "explicitList":
      retList := [[op(newList,i*op(newList,[0,3,2])+j) $ j = 1..op(newList,[0,3,2])] $ 
                 i = 0..op(newList,[0,2,2])-1];
      retList := float(retList):
      retDepVars := proposedOutVars:
      if bAnimated=TRUE then
        retParams := [op(tempIndets)]:
      end_if:
    elif op(newList,[0,2,2]) = 1 then
      for i from 1 to op(newList,[0,3,2]) do
        tempList := append(tempList,newList[1,i]):
      end_for:
    elif op(newList,[0,3,2]) = 1 then
      if bIsReallyConstant = TRUE and op(newList,[0,2,2]) = dim then
        retType := "explicitList":
        retList := [[op(newList,j) $ j = 1..dim]];
        retList := float(retList):
        ...
        for i from 1 to op(newList,[0,2,2]) do
          tempList := append(tempList,newList[i,1]):
        end_for:
      end_if:
    end_if:
  end_if:
  ...
  if nops(tempList) = 1 then
    [retType, tempIndets, tempDepVars, defList, retList] := doAnItem(tempList[1]):
  elif nops(tempList) > 0 then
    retList := []:
    tempIndets := []:
    tempDepVars := []:
    tempRetTypes := []:
    ...
    for i from 1 to nops(tempList) do
      rv := doAnItem(tempList[i]):
      tempRetTypes := append(tempRetTypes,rv[1]):
      for j in rv[2] do
        if contains(tempIndets,j) = 0 then
          tempIndets := append(tempIndets,j):
        end_if:
      end_for:
      if nops(rv[3]) > 0 then
        tempDepVars := append(tempDepVars, rv[3][1]):  //assume that only one can be returned?
      else
        tempDepVars := append(tempDepVars,0):
      end_if:
      tempDefList := tempDefList . rv[4]:
      tempRetList := tempRetList . rv[5]:
    end_for:
    retTypeSet := {op(tempRetTypes)}:
    if nops(retTypeSet) = 1 then
      retType := op(retTypeSet,1):
      if op(retTypeSet,1) = "functionImage" and nops(tempList) = dim then
        if (nops(tempDepVars) < dim) then   //have to fill in one!
          for j in proposedOutVars do
            if contains(tempDepVars,j) = 0 then
              tempDepVars := append(tempDepVars, j):
              tempDefList := append(tempDefList, [j,j]):
              if contains(tempIndets, j) = 0 then
                tempIndets := append(tempIndets, j):
              end_if:
              if (nops(tempDepVars) >= dim) then
                break:  //out of the enclosing "for" loop
              end_if:
            end_if:
          end_for:
        end_if:
      elif retType = "inequality" then
        retList := tempRetList:
      else
        retType := "incompatibleTypes":
      end_if:
      nMaxParams := adjustMaxParams(dim, retType, bAnimated):
      [retParams,defList,bParametric] := collateVars(tempIndets,tempDefList,nParamsNeeded,
                                                     nVarsNeeded,nMaxParams):

    else
      retType := "incompatibleTypes":  //Can't deal with these for now!
      nMaxParams := adjustMaxParams(dim, retType, bAnimated):
      [retParams,defList,bParametric] := collateVars(tempIndets,tempDefList,nParamsNeeded,
                                                      nVarsNeeded,nMaxParams):
    end_if:

    for i from 1 to nops(defList) do
      retDepVars := append(retDepVars, defList[i][1]):
      if retType = "functionImage" then
        retList := append(retList, defList[i][2]):
      end_if:
    end_for:

  end_if:
else
  //in this case, we're dealing with 1 expression!
  [retType, tempIndets, tempDepVars, defList, retList] := doAnItem(newList):
  if nops(tempDepVars) <> nops(defList) then
    bSomethingWrong := TRUE:
  else
    nMaxParams := adjustMaxParams(dim, retType, bAnimated):
    [retParams,defList,bParametric] := collateVars(tempIndets,defList,nParamsNeeded,
                                                   nVarsNeeded,nMaxParams):
  end_if:
  ...
  end_for:
end_if:
if (nops(retList) = 1) and ((retType = "functionGraph") or (retType = "functionImage")) then
  retList := op(retList,1):
end_if:
 
if retType = "functionImage" then
  case inType
    of complexImage do
      if nops(retList) > 1 then
        retList := [retList[nops(retList)]]:
      end_if:
    of vectorField do  //test dimensions just in case?
      if contains(extraArgs,gradient) <> 0 then
        retList := []:
      end_if:
      bParametric := FALSE:
    of curviLinear do
      retType := expr2text(inType):
    break:
    of stepped do
      retType := expr2text(inType):
      retList := []:
    break:
  end_case:
  if bParametric=TRUE then
    extraFlags := append(extraFlags, "parametric"):
  end_if:
end_if:
 
if nops(retList) = 0 or has(retList,FAIL) then
  [retType, retParams, retDepVars]
else
  retList := SWreplaceProcNames(retList):
  retList := text2expr(retList):
  [retType, retParams, retDepVars, retList,extraFlags]
end_if:
end_proc:
//End of swPreparePlot30
//***************************************************************//
