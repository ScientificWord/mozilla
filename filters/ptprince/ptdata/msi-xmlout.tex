...
\def\includegraphics{%
  \@ifnextchar[{\includegrA}{\includegrB}}

\def\includegrA[#1]#2{%
  \msigraphic{#2}%
  \msidefault{\includegraphics}[#1]{#2}}

\def\includegrB#1{%
  \msigraphic{#1}%
  \msidefault{\includegraphics}{#1}}
...
\def\msimakecss{
   ...
   \write\css{acknowledgment>*:first-child:before{}}  
   ...
   \write\css{texnote>*:first-child:before{}}
   ...
   \write\css{solution>*:first-child:before{}}
...
\def\msi@empty@elmA[#1]#2#3{%
     \expandafter\def\csname #2\endcsname{%
        \ifmmode
           %\msitag{\msipassthru{<mspace width="thickmathspace" type="#2"/>}}%
...
\def\cite{\@ifnextchar*{\citestar}{\citenostar}}
...
\def\citestar#1#2{
   ...
   \begingroup
   \ifmmode
    \msitag{\msipassthru{<citation xmlns="http://www.w3.org/1999/xhtml" citekey="#2" hasRemark="false" isStarred="true"></citation>}}%
  \else
    \msiopentag{citation}{<citation xmlns="http://www.w3.org/1999/xhtml" 
                                    citekey="}#2\msitag{" hasRemark="false" isStarred="true">}%
    \msiclosetag{citation}{</citation>}%
  \fi
  \endgroup  
}
...
\def\out@itema[#1]{
   ...
   \fi
   \def\startitemtag##1{\msiopentag{##1}{<##1>}\msiopentag{bodyText}{<bodyText>}}%
   ...
   \else
     \ifx\itemprefix\@enumerate
       \msiopentag{numberedLabel}{<numberedLabel>}#1\msiclosetag{numberedLabel}{</numberedLabel>}%
	 \else
...
%Initialize \bibliographystyle in case it is preceded by \bibliography command
\def\@bibstyle{
  \msihmode
  \msitag{}}
...
\def\marginpar{%
 \bgroup
  \catcode`\&=4
  \@ifnextchar[\@marginparA\@marginparB}
...
\def\setlength#1#2{%
  \msihmode
  \msitag{<texb xmlns="http://www.w3.org/1999/xhtml" enc="1" name="}\msiattrib{\setlength #1}\msitag{"><![CDATA[\setlength#1{#2}]]></texb>}%
}
...
\bgroup
  ...
  \gdef\@scan@matrix#1\end{matrix}[%
     \egroup 
     \@make@braces@normal
     \msitag[\begin{matrix}#1\end{matrix}]
  ]
  ...
  \gdef\@scan@cases#1\end{cases}[%
     \egroup
     \@make@braces@other
     \msitag[\begin{cases}#1\end{cases}]
     %\temptoksa=[\begin{cases}#1\end{cases}]%
     %\the\temptoksa
  ]
  \gdef\@scan@rcases#1\end{rcases}[%
     \egroup
     \@make@braces@other
     \msitag[\begin{rcases}#1\end{rcases}]
     %\temptoksa=[\begin{rcases}#1\end{rcases}]%
     %\the\temptoksa
  ]
  \gdef\@scan@bsmallmatrix#1\end{bsmallmatrix}[%
     \egroup
     \@make@braces@normal
     \msitag[\begin{bsmallmatrix}#1\end{bsmallmatrix}]
     %\temptoksa=[\begin{bsmallmatrix}#1\end{bsmallmatrix}]%
     %\the\temptoksa
  ]
  \gdef\@scan@Bsmallmatrix#1\end{Bsmallmatrix}[%
     \egroup
  ...
  \gdef\@scan@vsmallmatrix#1\end{vsmallmatrix}[%
     \egroup
     \@make@braces@normal
     \msitag[\begin{vsmallmatrix}#1\end{vsmallmatrix}]
     %\temptoksa=[\begin{vsmallmatrix}#1\end{vsmallmatrix}]%
     %\the\temptoksa
  ]
  \gdef\@scan@Vsmallmatrix#1\end{Vsmallmatrix}[%
     \egroup
  ...
  \gdef\@scan@psmallmatrix#1\end{psmallmatrix}[%
     \egroup
     \@make@braces@normal
     \msitag[\begin{psmallmatrix}#1\end{psmallmatrix}]
     %\temptoksa=[\begin{psmallmatrix}#1\end{psmallmatrix}]%
     %\the\temptoksa
  ]
\egroup
...
\def\msi@begin@matrix{\@make@braces@other\relax\@scan@matrix} 
...
\def\msi@begin@cases{\@make@braces@other\relax\@scan@cases}
\def\msi@begin@rcases{\@make@braces@other\relax\@scan@rcases}
\def\msi@begin@bsmallmatrix{\@make@braces@other\relax\@scan@bsmallmatrix} 
\def\msi@begin@Bsmallmatrix{\@make@braces@other\relax\@scan@Bsmallmatrix} 
\def\msi@begin@vsmallmatrix{\@make@braces@other\relax\@scan@vsmallmatrix} 
\def\msi@begin@Vsmallmatrix{\@make@braces@other\relax\@scan@Vsmallmatrix} 
\def\msi@begin@psmallmatrix{\@make@braces@other\relax\@scan@psmallmatrix}
...
\def\verb{\@ifnextchar*{\verbstar}{\verbnostar}}
...
\def\verbnostar@egroup{\msiclosetag{verb}{</verb>}\egroup}
\def\@sverb#1{%
  \catcode`#1\active
  \lccode`\~`#1\relax
  \lowercase{\let~\verbnostar@egroup}%
}
...
\def\verbstar{
   ...
   \bgroup
      ...
      \catcode`\<=\active
      \catcode`\%=\other      
      \msiopentag{verbstar}{<verbstar>}%
      \@stverb
}

\def\verbstar@egroup{\msiclosetag{verbstar}{</verbstar>}\egroup}
\def\@stverb#1#2{%
  \catcode`#2\active
  \lccode`\~`#2\relax
  \lowercase{\let~\verbstar@egroup}%
}
...
\def\@cdmathring@{\hex{30A}} %combining ring above
\def\@cddddot@{\hex{20DB}}   %triple dots
\def\@cdddddot@{\hex{20DC}}  %four dots
...
\def\mathring#1{\@accent{#1}{mathring}}

\def\dddot#1{\@accent{#1}{dddot}}

\def\ddddot#1{\@accent{#1}{ddddot}}
...
\def\medspace{%
   \ifmmode
      \string\medspace
   \else
      \msihmode\msitag{<hspace type="medSpace"/>}%
   \fi
}
%\let\:\medspace
...
\def\@label#1{%
  \endgroup
   ...
   \else
      \msihmode\msitag{<a name="}\msitoxml{#1}\msitag{" key="}\msitoxml{#1}\msitag{" id="}\msitoxml{#1}\msitag{"/>}%
   \fi}
...
\def\vskip{
  \msihmode
  \msitag{<texb enc="1" name="vskip">\vskip</texb>}
}

\def\hskip{
  \msihmode
  \msitag{<texb enc="1" name="hskip">\hskip</texb>}
}

\def\topskip{
  \msihmode
  \msitag{<texb enc="1" name="topskip">\topskip</texb>}
}
...
\def\@@TeXButton#1#2{%
   \ifmmode
     \msitag{\msipassthru}\char`\{\msitag{<texb xmlns="http://www.w3.org/1999/xhtml" enc="1" name="#1"><![CDATA[#2]]></texb>}\char`\}
   \else
     \msihmode
...
\def\caption{\@ifnextchar[\msi@captiona\msi@captionb}

\def\msi@captiona[#1]#2{%
  \msiopentag{imagecaption}{<imagecaption align="bottom">}%
    \msiopentag{shortCaption}{<shortTitle>}%
      #1%
    \msiclosetag{shortCaption}{</shortTitle>}%
    #2
