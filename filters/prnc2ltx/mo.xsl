<xsl:stylesheet version="1.1" xmlns:exsl="http://exslt.org/common" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:regexp="http://exslt.org/regular-expressions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <!-- Algorithm
...
-->
    ...
    <xsl:choose>
      <xsl:when test="@msimathname='true'">
        <xsl:choose>
          ...
          <xsl:otherwise>\ensuremath{\operatorname*{
            <xsl:apply-templates/>
            }}</xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
        ...
        <xsl:if test="not(@largeop='true')">
          <xsl:choose>
            ...
            <xsl:when test="normalize-space(string())='〈'             or              normalize-space(string())='〈'">
              <xsl:call-template name="translate-fencing-mo">
            ...
            <!--       <xsl:when test="normalize-space(string())='&#x250A;'"><xsl:call-template name="translate-fencing-mo"><xsl:with-param name="LaTeX-fence-token" select="'&#x002E;'"/></xsl:call-template></xsl:when>-->
  ...
  We don't need a special translation here - default gives same outputnormalize-space(string())
      ...
      <xsl:when test="normalize-space(string())='d'"><xsl:if test="@lspace='0normalize-space(string())333333em'"><xsl:text>\,</xsl:text></xsl:if><xsl:text>d</xsl:text></xsl:when>-->
            <!-- a few special cases -->
            ...
            <xsl:otherwise>
              <xsl:variable name="LaTeX-symbols">
                <xsl:call-template name="chars-to-LaTeX-Math">
                  <xsl:with-param name="unicode-cdata" select="substring(normalize-space(string())"/>
                </xsl:call-template>
                <xsl:apply-templates />
              </xsl:variable>
              <xsl:choose>
...
-->
  ...
  <xsl:template name="bigop">
    ...
    <xsl:choose>
      <xsl:when test="((@largeop='true') or (@stretchy='true'))">
        <!-- BBM for bug 3094
        <xsl:choose><xsl:when test="ancestor-or-self::*/@displaystyle='true'"><xsl:text xml:space="preserve">\mathop{</xsl:text><xsl:text>\displaystyle\</xsl:text><xsl:text>\</xsl:text><xsl:value-of select="$LaTeX-nom"/><xsl:text xml:space="preserve">}</xsl:text></xsl:when><xsl:otherwise>-->
        <xsl:text>\</xsl:text>
  ...
  <xsl:template name="count-letters">
    ...
    <xsl:variable name="first-char" select="substring($unicode-cdata,1,1)"/>
    <xsl:choose>
      <xsl:when test="          $first-char = 'A'       or $first-char = 'B'       or $first-char = 'C'       or $first-char = 'D'       or $first-char = 'E'       or $first-char = 'F'       or $first-char = 'G'       or $first-char = 'H'       or $first-char = 'I'       or $first-char = 'J'       or $first-char = 'K'       or $first-char = 'L'       or $first-char = 'M'       or $first-char = 'N'       or $first-char = 'O'       or $first-char = 'P'       or $first-char = 'Q'       or $first-char = 'R'       or $first-char = 'S'       or $first-char = 'T'       or $first-char = 'U'       or $first-char = 'V'       or $first-char = 'W'       or $first-char = 'X'       or $first-char = 'Y'       or $first-char = 'Z'       or $first-char = 'a'       or $first-char = 'b'       or $first-char = 'c'       or $first-char = 'd'       or $first-char = 'e'       or $first-char = 'f'       or $first-char = 'g'       or $first-char = 'h'       or $first-char = 'i'       or $first-char = 'j'       or $first-char = 'k'       or $first-char = 'l'       or $first-char = 'm'       or $first-char = 'n'       or $first-char = 'o'       or $first-char = 'p'       or $first-char = 'q'       or $first-char = 'r'       or $first-char = 's'       or $first-char = 't'       or $first-char = 'u'       or $first-char = 'v'       or $first-char = 'w'       or $first-char = 'x'       or $first-char = 'y'       or $first-char = 'z'">
        <xsl:choose>
  ...
  <xsl:template name="is-all-caps">
    <xsl:param name="unicode-cdata"/>
    <!-- xsl:choose><xsl:when test="regexp:test(normalize-space(string($unicode-cdata)),'[^A-Z]+','g')" -->
    <xsl:text>false</xsl:text>
    <!--/xsl:when><xsl:otherwise><xsl:text>true</xsl:text></xsl:otherwise></xsl:choose -->
    <!--xsl:variable name="first-char" select="substring($unicode-cdata,1,1)"/><xsl:choose><xsl:when test="
         $first-char = 'A'
      or $first-char = 'B'
      or $first-char = 'C'
      or $first-char = 'D'
      ...
      or $first-char = 'X'
      or $first-char = 'Y'
      or $first-char = 'Z'"><xsl:choose><xsl:when test="string-length($unicode-cdata)&gt;1"><xsl:call-template name="is-all-caps"><xsl:with-param name="unicode-cdata" select="substring($unicode-cdata,2)"/></xsl:call-template></xsl:when><xsl:otherwise><xsl:text>true</xsl:text></xsl:otherwise></xsl:choose></xsl:when><xsl:otherwise><xsl:text>false</xsl:text></xsl:otherwise></xsl:choose-->
  </xsl:template>
</xsl:stylesheet>