...
<!--     <xsl:text>8. curr-mo-form is </xsl:text><xsl:value-of select="$curr-mo-form"/>
 -->
    <xsl:choose> <!-- Here we look for a following sibling that could represent the closing delimiter of our possible fence. -->
 ...
 when when the object on its right is scripted. -->
      ...
      <xsl:when test="$curr-mo-stretchy!='false'
          ...
          or              $left-mo-form='ambiguous') "> <!-- set up variable re fence body - delimiter may be absorbed -->
