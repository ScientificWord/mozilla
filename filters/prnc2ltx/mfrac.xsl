...
When the first
<mo>
  is encountered, we look ahead to see if
  it may be absorbed in the LaTeX output a following object.
  For example, the output may be \binom or \genfrac{}{}{}{}{}{}.

  When
                            <mfrac>
                              is encountered, we must look at it's siblings
  (and in some cases, the sibling of
                              <mrow>
                                ancestors) to see
  ...
  When the second
                                <mo>
                                  is encountered, we look back to see if
  ...
  and
  <mfrac>
  .  The related decisions must
  be made ( and maintained ) in both scripts.
  -->
  <!-- 4 variables to hold info about a possible fencing <mo>
  to the immediate left of the current
  <mfrac>
    -->

  <xsl:text>Starting mfrac</xsl:text>
  ...
  <!-- 4 variables to hold info about a possible fencing <mo>
  to the immediate right of the current
  <mfrac>
    -->
  ...
  <xsl:value-of select="$frac-linethickness"/>
  <xsl:text>-BBB-</xsl:text>
  <xsl:value-of select="$frac-delims-l-delim"/>
  <xsl:text>-AAA</xsl:text>
  ...
  and        not($frac-delims-r-delim='false')">
    ...
    <xsl:otherwise>
      <xsl:text>mfracs with default line thickness</xsl:text> <!-- BBM -->
      <xsl:choose>
  ...
  and        not($frac-delims-r-delim='false')">
        ...
        <xsl:otherwise>
          <xsl:text>writing out frac</xsl:text>  <!-- BBM -->
          <xsl:choose>
            <xsl:when test="$frac-context-size='d'">
  ...
  <xsl:text select="$frac-delims-l-delim"/> <!-- BBM -->
  <xsl:text select="$lthick-attr-val"/> <!-- BBM -->
