...
let gComposerJSCommandControllerID = 0

// -----------------------------------------------------------------------------------
function SetupHTMLEditorCommands () {
  const commandTable = GetComposerCommandTable()
...
function SetupComposerWindowCommands () {
  ...
  if (gComposerWindowControllerID) { return }
  ...
  const windowControllers = window.controllers
  ...
  try {
    ...
    const interfaceRequestor = composerController.QueryInterface(Components.interfaces.nsIInterfaceRequestor)
...
function GetComposerCommandTable () {
  ...
  if (controller) {
    const interfaceRequestor = controller.QueryInterface(Components.interfaces.nsIInterfaceRequestor)
...
function goUpdateCommandState (command) {
  try {
    const controller = top.document.commandDispatcher.getControllerForCommand(command)
    if (!(controller instanceof Components.interfaces.nsICommandController)) { return }
...
function goUpdateComposerMenuItems (commandset) {
  ...
  for (let i = 0; i < commandset.childNodes.length; i++) {
    const commandNode = commandset.childNodes[i]
    const commandID = commandNode.id
    if (commandID) {
  ...
  try {
    ...
    if (controller && controller.isCommandEnabled(command)) {
      if (controller instanceof Components.interfaces.nsICommandController) {
        ...
        if (params) { controller.getCommandStateWithParams(command, params) }
      } else {
...
function pokeStyleUI (uiID, aDesiredState) {
  try {
    const commandNode = top.document.getElementById(uiID)
    if (!commandNode) { return }

    const uiState = (commandNode.getAttribute('state') == 'true')
    if (aDesiredState != uiState) {
      let newState
      if (aDesiredState) { newState = 'true' } else { newState = 'false' }
...
function doStyleUICommand (cmdStr) {
  try {
    const cmdParams = newCommandParams()
    goDoCommandParams(cmdStr, cmdParams)
    if (cmdParams) { pokeStyleUI(cmdStr, cmdParams.getBooleanValue('state_all')) }
...
function pokeMultiStateUI (uiID, cmdParams) {
  try {
    ...
    const isMixed = cmdParams.getBooleanValue('state_mixed')
    let desiredAttrib
    if (isMixed) { desiredAttrib = 'mixed' } else { desiredAttrib = cmdParams.getCStringValue('state_attribute') }
...
function doStatefulCommand (commandID, newState) {
  ...
  if (commandNode) { commandNode.setAttribute('state', newState) }
...
var nsSaveAsCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    if (editor) {
      FinishHTMLSource()
      const result = SaveDocument(true, false, editor.contentsMIMEType)
      window.content.focus()
      return result
    }
    return false
...
var nsExportToTextCommand =
{
  ...
  doCommand: function (aCommand) {
    if (GetCurrentEditor()) {
      FinishHTMLSource()
      const result = SaveDocument(true, true, 'text/plain')
      window.content.focus()
      return result
    }
    return false
  ...
  doCommand: function (aCommand) {
    if (GetCurrentEditor()) {
      ...
      try {
        ...
        if (prefs) { showPublishDialog = prefs.getBoolPref('editor.always_show_publish_dialog') }
      } catch (e) {}
...
var nsPublishAsCommand =
{
  ...
  doCommand: function (aCommand) {
    if (GetCurrentEditor()) {
      ...
      window.content.focus()
      if (window.ok) { return Publish(publishData) }
    }
    return false
  }
}
...
function GetExtensionBasedOnMimeType (aMIMEType) {
  try {
    ...
    let fileExtension = mimeService.getPrimaryExtension(aMIMEType, null)
...
function GetSuggestedFileName (aDocumentURLString, aMIMEType) {
  let extension = GetExtensionBasedOnMimeType(aMIMEType)
  ...
  if (aDocumentURLString.length >= 0 && !IsUrlAboutBlank(aDocumentURLString)) {
    ...
    try {
      ...
      // grab the file name
      const url = docURI.fileBaseName
      if (url) { return url + extension }
    } catch (e) {}
  }
...
function PromptForSaveLocation (aDoSaveAsText, aEditorType, aMIMEType, aDocumentURLString) {
  ...
  let promptString
  if (aDoSaveAsText || aEditorType == 'text') { promptString = GetString('ExportToText') } else { promptString = GetString('SaveDocumentAs') }

  fp.init(window, promptString, nsIFilePicker.modeSave)

  // Set filters according to the type of output
  if (aDoSaveAsText) { fp.appendFilters(nsIFilePicker.filterText) } else { fp.appendFilters(nsIFilePicker.filterHTML) }
  fp.appendFilters(nsIFilePicker.filterAll)

  // now let's actually set the filepicker's suggested filename
  const suggestedFileName = GetSuggestedFileName(aDocumentURLString, aMIMEType)
  if (suggestedFileName) { fp.defaultString = suggestedFileName }
  ...
  try {
    ...
    try {
      const docURI = ioService.newURI(aDocumentURLString, GetCurrentEditor().documentCharacterSet, null)
      isLocalFile = docURI.schemeIs('file')
    } catch (e) {}

    let parentLocation = null
    if (isLocalFile) {
      const fileLocation = fileHandler.getFileFromURLSpec(aDocumentURLString) // this asserts if url is not local
      parentLocation = fileLocation.parent
    }
    if (parentLocation) {
      // Save current filepicker's default location
      if ('gFilePickerDirectory' in window) { gFilePickerDirectory = fp.displayDirectory }
  ...
  } else if ('gFilePickerDirectory' in window && gFilePickerDirectory) { fp.displayDirectory = gFilePickerDirectory }
...
function PromptAndSetTitleIfNone () {
  ...
  const confirmed = promptService.prompt(window, captionStr, msgStr, result, null, { value: 0 })
  if (confirmed) { SetDocumentTitle(TrimString(result.value)) }
...
function OutputFileWithPersistAPI (editorDoc, aDestinationLocation, aRelatedFilesParentDir, aMimeType) {
  ...
  const editor = GetCurrentEditor()
  try {
    const imeEditor = editor.QueryInterface(Components.interfaces.nsIEditorIMESupport)
    imeEditor.ForceCompositionEnd()
  } catch (e) {}

  let isLocalFile = false
  try {
    const tmp1 = aDestinationLocation.QueryInterface(Components.interfaces.nsIFile)
    isLocalFile = true
  } catch (e) {
    try {
      const tmp = aDestinationLocation.QueryInterface(Components.interfaces.nsIURI)
  ...
  try {
    ...
    const persistObj = Components.classes['@mozilla.org/embedding/browser/nsWebBrowserPersist;1'].createInstance(webPersist)
    persistObj.progressListener = gEditorOutputProgressListener

    const wrapColumn = GetWrapColumn()
    let outputFlags = GetOutputFlags(aMimeType, wrapColumn)
...
function GetOutputFlags (aMimeType, aWrapColumn) {
  let outputFlags = 0
  const editor = GetCurrentEditor()
  let outputEntity = (editor && editor.documentCharacterSet == 'ISO-8859-1')
  ...
  if (aMimeType == 'text/plain') {
    ...
    try {
      ...
      const prefs = GetPrefs()
      if (prefs.getBoolPref('editor.prettyprint')) { outputFlags |= webPersist.ENCODE_FLAGS_FORMATTED }

      // How much entity names should we output? Check the pref
      const encodeEntity = prefs.getCharPref('editor.encode_entity')
      switch (encodeEntity) {
  ...
  if (aWrapColumn > 0) { outputFlags |= webPersist.ENCODE_FLAGS_WRAP }
...
var gEditorOutputProgressListener =
{
  onStateChange: function (aWebProgress, aRequest, aStateFlags, aStatus) {
    ...
    let requestSpec
    try {
      const channel = aRequest.QueryInterface(nsIChannel)
      requestSpec = StripUsernamePasswordFromURI(channel.URI)
    } catch (e) {
      if (gShowDebugOutputStateChange) { dump('***** onStateChange; NO REQUEST CHANNEL\n') }
    }

    let pubSpec
    if (gPublishData) { pubSpec = gPublishData.publishUrl + gPublishData.docDir + gPublishData.filename }

    if (gShowDebugOutputStateChange) {
      dump('\n***** onStateChange request: ' + requestSpec + '\n')
      dump('      state flags: ')

      if (aStateFlags & nsIWebProgressListener.STATE_START) { dump(' STATE_START, ') }
      if (aStateFlags & nsIWebProgressListener.STATE_STOP) { dump(' STATE_STOP, ') }
      if (aStateFlags & nsIWebProgressListener.STATE_IS_NETWORK) { dump(' STATE_IS_NETWORK ') }
    ...
    if (aStateFlags & nsIWebProgressListener.STATE_STOP) {
      ...
      try {
        ...
        const httpChannel = aRequest.QueryInterface(Components.interfaces.nsIHttpChannel)
        const httpResponse = httpChannel.responseStatus
        if (httpResponse < 200 || httpResponse >= 300) { aStatus = httpResponse } // not a real error but enough to pass check below
        else if (aStatus == kErrorBindingAborted) { aStatus = 0 }

        if (gShowDebugOutputStateChange) { dump('http response is: ' + httpResponse + '\n') }
      } catch (e) {
        if (aStatus == kErrorBindingAborted) { aStatus = 0 }
      }

      // We abort publishing for all errors except if image src file is not found
      const abortPublishing = (aStatus != 0 && aStatus != kFileNotFound)
      ...
      if (aStateFlags & nsIWebProgressListener.STATE_IS_NETWORK &&
        ...
        if (gPersistObj.result == 0) {
          ...
          try {
            ...
            const docUrl = GetDocUrlFromPublishData(gPublishData)
          ...
          if (gPublishData) {
            ...
            } else { SavePassword(gPublishData) }
          }
  ...
  QueryInterface: function (aIID) {
    ...
    aIID.equals(Components.interfaces.nsIAuthPrompt)) { return this }
    throw Components.results.NS_NOINTERFACE
  },

  // nsIPrompt
  alert: function (dlgTitle, text) {
    AlertWithTitle(dlgTitle, text, gProgressDialog || window)
  },
  ...
  confirmCheck: function (dlgTitle, text, checkBoxLabel, checkObj) {
    const promptServ = GetPromptService()
    if (!promptServ) { return }
  ...
  promptPassword: function (dlgTitle, text, pwObj, checkBoxLabel, savePWObj) {
    ...
    try {
      ...
      if (gPublishData) { savePWObj.value = gPublishData.savePassword }

      ret = promptServ.promptPassword(gProgressDialog || window,
        dlgTitle, text, pwObj, checkBoxLabel, savePWObj)

      if (!ret) { setTimeout(CancelPublishing, 0) }

      if (ret && gPublishData) { UpdateUsernamePasswordFromPrompt(gPublishData, gPublishData.username, pwObj.value, savePWObj.value) }
    } catch (e) {}

    return ret
  },
  promptUsernameAndPassword: function (dlgTitle, text, userObj, pwObj, checkBoxLabel, savePWObj) {
    const ret = PromptUsernameAndPassword(dlgTitle, text, savePWObj.value, userObj, pwObj)
    if (!ret) { setTimeout(CancelPublishing, 0) }
  ...
  prompt: function (dlgTitle, text, pwrealm, savePW, defaultText, result) {
    ...
    const savePWObj = { value: savePW }
    const ret = promptServ.prompt(gProgressDialog || window,
  ...
  promptUsernameAndPassword: function (dlgTitle, text, pwrealm, savePW, userObj, pwObj) {
    const ret = PromptUsernameAndPassword(dlgTitle, text, savePW, userObj, pwObj)
    if (!ret) { setTimeout(CancelPublishing, 0) }
    return ret
  },
...
function PromptUsernameAndPassword (dlgTitle, text, savePW, userObj, pwObj) {
  ...
  try {
    ...
    if (gPublishData) {
      ...
      if (!userObj.value) { userObj.value = gPublishData.username }
    }

    ret = promptServ.promptUsernameAndPassword(gProgressDialog || window,
      dlgTitle, text, userObj, pwObj,
      GetString('SavePassword'), savePWObj)
    if (ret && gPublishData) { UpdateUsernamePasswordFromPrompt(gPublishData, userObj.value, pwObj.value, savePWObj.value) }
  } catch (e) {}
...
function IsSupportedTextMimeType (aMimeType) {
  for (let i = 0; i < kSupportedTextMimeTypes.length; i++) {
    if (kSupportedTextMimeTypes[i] == aMimeType) { return true }
  }
  return false
}
...
function SaveDocument (aSaveAs, aSaveCopy, aMimeType) {
  const editor = GetCurrentEditor()
  if (!aMimeType || aMimeType == '' || !editor) { throw NS_ERROR_NOT_INITIALIZED }

  const editorDoc = editor.document
  if (!editorDoc) { throw NS_ERROR_NOT_INITIALIZED }
  ...
  if (editorType != 'text' && editorType != 'html' &&
      editorType != 'htmlmail' && editorType != 'textmail') { throw NS_ERROR_NOT_IMPLEMENTED }

  const saveAsTextFile = IsSupportedTextMimeType(aMimeType)

  // check if the file is to be saved is a format we don't understand; if so, bail
  if (aMimeType != 'text/html' && aMimeType != 'application/xhtml+xml' && !saveAsTextFile) { throw NS_ERROR_NOT_IMPLEMENTED }

  if (saveAsTextFile) { aMimeType = 'text/plain' }

  let urlstring = GetDocumentUrl()
  let mustShowFileDialog = (aSaveAs || IsUrlAboutBlank(urlstring) || (urlstring == ''))

  // If editing a remote URL, force SaveAs dialog
  if (!mustShowFileDialog && GetScheme(urlstring) != 'file') { mustShowFileDialog = true }

  let replacing = !aSaveAs
  const titleChanged = false
  let doUpdateURI = false
  ...
  if (mustShowFileDialog) {
    try {
      ...
      if (!saveAsTextFile && (editorType == 'html')) {
        const userContinuing = PromptAndSetTitleIfNone() // not cancel
        if (!userContinuing) { return false }
      }

      const dialogResult = PromptForSaveLocation(saveAsTextFile, editorType, aMimeType, urlstring)
      if (dialogResult.filepickerClick == nsIFilePicker.returnCancel) { return false }
  ...
  try {
    ...
    if (!tempLocalFile) {
      ...
      if (docURI.schemeIs('file')) {
        const fileHandler = GetFileProtocolHandler()
    ...
    let relatedFilesDir = null

    // First check pref for saving associated files
    let saveAssociatedFiles = false
    try {
      const prefs = GetPrefs()
    ...
    if (saveAssociatedFiles && aSaveAs) {
      try {
        if (tempLocalFile) {
          ...
          let oldLocation = GetDocumentUrl()
          const oldLocationLastSlash = oldLocation.lastIndexOf('\/')
          if (oldLocationLastSlash != -1) { oldLocation = oldLocation.slice(0, oldLocationLastSlash) }

          let relatedFilesDirStr = urlstring
          const newLocationLastSlash = relatedFilesDirStr.lastIndexOf('\/')
          if (newLocationLastSlash != -1) { relatedFilesDirStr = relatedFilesDirStr.slice(0, newLocationLastSlash) }
          if (oldLocation == relatedFilesDirStr || IsUrlAboutBlank(oldLocation)) { relatedFilesDir = null } else { relatedFilesDir = tempLocalFile.parent }
        } else {
          const lastSlash = urlstring.lastIndexOf('\/')
          if (lastSlash != -1) {
            const relatedFilesDirString = urlstring.slice(0, lastSlash + 1) // include last slash
            ioService = GetIOService()
    ...
    if (tempLocalFile) { destinationLocation = tempLocalFile } else { destinationLocation = docURI }
  ...
  if (success) {
    try {
      if (doUpdateURI) {
        ...
        if (tempLocalFile) { docURI = GetFileProtocolHandler().newFileURI(tempLocalFile) }
      ...
      if (!aSaveCopy) { editor.resetModificationCount() }
      // this should cause notification to listeners that document has changed
    ...
    const saveDocStr = GetString('SaveDocument')
    const failedStr = GetString('SaveFileFailed')
...
function Publish (publishData) {
  ...
  if (gPublishData.publishOtherFiles) { gPublishData.otherFilesURI = CreateURIFromPublishData(publishData, false) } else { gPublishData.otherFilesURI = null }
  ...
  if (GetScheme(publishData.publishUrl) == 'ftp' && !publishData.username) {
    const message = GetString('PromptFTPUsernamePassword').replace(/%host%/, GetHost(publishData.publishUrl))
    const savePWobj = { value: publishData.savePassword }
    const userObj = { value: publishData.username }
    const pwObj = { value: publishData.password }
    if (!PromptUsernameAndPassword(GetString('Prompt'), message, savePWobj, userObj, pwObj)) { return false } // User canceled out of dialog
...
function FinishPublishing () {
  ...
  if (gCommandAfterPublishing) {
    ...
    const command = gCommandAfterPublishing
    gCommandAfterPublishing = null
    goDoCommand(command)
  }
}
...
function CreateURIFromPublishData (publishData, doDocUri) {
  if (!publishData || !publishData.publishUrl) { return null }

  let URI
  try {
    let spec = publishData.publishUrl
    if (doDocUri) { spec += FormatDirForPublishing(publishData.docDir) + publishData.filename } else { spec += FormatDirForPublishing(publishData.otherDir) }

    const ioService = GetIOService()
    URI = ioService.newURI(spec, GetCurrentEditor().documentCharacterSet, null)

    if (publishData.username) { URI.username = publishData.username }
    if (publishData.password) { URI.password = publishData.password }
  } catch (e) {}
...
function GetDocUrlFromPublishData (publishData) {
  if (!publishData || !publishData.filename || !publishData.publishUrl) { return '' }

  // If user was previously editing an "ftp" url, then keep that as the new scheme
  let url
  const docScheme = GetScheme(GetDocumentUrl())
  ...
  if (!GetScheme(publishData.browseUrl)) { url = publishData.publishUrl } else { url = publishData.browseUrl }

  url += FormatDirForPublishing(publishData.docDir) + publishData.filename

  if (GetScheme(url) == 'ftp') { url = InsertUsernameIntoUrl(url, publishData.username) }
...
var nsRevertCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    if (promptService) {
      ...
      if (!title) { title = GetString('untitled') }

      const msg = GetString('AbandonChanges').replace(/%title%/, title)

      const result = promptService.confirmEx(window, GetString('RevertCaption'), msg,
...
function CloseWindow () {
  ...
  if (document.getElementById('preview-frame')) { document.getElementById('preview-frame').loadURI('about:blank') }
  if (CheckAndSaveDocument('cmd_close', true)) {
    if (window.InsertCharWindow) { SwitchInsertCharToAnotherEditorOrClose() }

    try {
      const basewin = window.QueryInterface(Components.interfaces.nsIInterfaceRequestor)
...
var nsPreviewCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    if (!CheckAndSaveDocument('cmd_preview', DocumentHasBeenSaved())) { return }
    ...
    if (DocumentHasBeenSaved()) {
      ...
      try {
        ...
        const windowManagerInterface = windowManager.QueryInterface(Components.interfaces.nsIWindowMediator)
        const enumerator = windowManagerInterface.getEnumerator('navigator:browser')

        var documentURI = GetDocumentUrl()
        while (enumerator.hasMoreElements()) {
          browser = enumerator.getNext().QueryInterface(Components.interfaces.nsIDOMWindowInternal)
          if (browser && (documentURI == browser.getBrowser().currentURI.spec)) { break }
...
var nsSendPageCommand =
{
  ...
  doCommand: function (aCommand) {
    // Don't continue if user canceled during prompt for saving
    // DocumentHasBeenSaved will test if we have a URL and suppress "Don't Save" button if not
    if (!CheckAndSaveDocument('cmd_editSendPage', DocumentHasBeenSaved())) { return }
...
var nsFindAgainCommand =
{
  ...
  doCommand: function (aCommand, editorElement) {
    try {
      ...
      const findInst = editorElement.webBrowserFind
      const findService = Components.classes['@mozilla.org/find/find_service;1']
...
var nsSpellingCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    try {
      const skipBlockQuotes = (window.document.firstChild.getAttribute('windowtype') == 'msgcompose')
...
var nsValidateCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    if (IsDocumentModified() || IsHTMLSourceChanged()) {
      if (!CheckAndSaveDocument('cmd_validate', false)) { return }
    ...
    if (ifile) {
      ...
      const vwin = window.open('http://validator.w3.org/file-upload.html',
        'EditorValidate')
      ...
      const vwin2 = window.open('http://validator.w3.org/check?uri=' +
                              URL2Validate +
                              '&doctype=Inline',
      'EditorValidate')
...
var nsLabelCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    try {
      ...
      let labelElement = editor.getSelectedElement(tagName)
      if (!labelElement) { labelElement = editor.getElementOrParentByTagName(tagName, editor.selection.anchorNode) }
      if (!labelElement) { labelElement = editor.getElementOrParentByTagName(tagName, editor.selection.focusNode) }
      if (labelElement) {
...
var nsIsIndexCommand =
{
  ...
  doCommand: function (aCommand) {
    try {
      const editor = GetCurrentEditor()
      const isindexElement = editor.createElementWithDefaults('isindex')
  ...
  doCommand: function (aCommand) {
    ...
    if (hLine) {
      ...
      try {
        ...
        const prefs = GetPrefs()
        const align = prefs.getIntPref('editor.hrule.align')
        if (align == 0) { editor.setAttributeOrEquivalent(hLine, 'align', 'left', true) } else if (align == 2) { editor.setAttributeOrEquivalent(hLine, 'align', 'right', true) }

        // Note: Default is center (don't write attribute)

        let width = prefs.getIntPref('editor.hrule.width')
        const percent = prefs.getBoolPref('editor.hrule.width_percent')
        if (percent) { width = width + '%' }

        editor.setAttributeOrEquivalent(hLine, 'width', width, true)

        const height = prefs.getIntPref('editor.hrule.height')
        editor.setAttributeOrEquivalent(hLine, 'size', String(height), true)

        const shading = prefs.getBoolPref('editor.hrule.shading')
        if (shading) { hLine.removeAttribute('noshade') } else { hLine.setAttribute('noshade', 'noshade') }
...
var nsLinkCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    const element = GetObjectForProperties()
    if (element && element.nodeName.toLowerCase() == 'img') { window.openDialog('chrome://editor/content/EdImageProps.xul', '_blank', 'chrome,close,titlebar,modal', null, true) } else { window.openDialog('chrome://editor/content/EdLinkProps.xul', '_blank', 'chrome,close,titlebar,modal') }
    window.content.focus()
  }
}
...
var nsObjectPropertiesCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    let element = GetObjectForProperties()
    if (element) {
      ...
      switch (name) {
        ...
        case 'input':
          ...
          if (type && type.toLowerCase() == 'image') { goDoCommand('cmd_inputimage') } else { goDoCommand('cmd_inputtag') }
          break
        case 'textarea':
      ...
      if (element) { goDoCommand('cmd_link') }
    }
    window.content.focus()
  }
}
...
var nsSetSmiley =
{
  ...
  getCommandStateParams: function (aCommand, aParams, aRefCon) {},
  doCommandParams: function (aCommand, aParams, aRefCon) {
    let smileyCode = aParams.getCStringValue('state_attribute')
    ...
    try {
      ...
      const extElement = editor.createElementWithDefaults('span')
      extElement.setAttribute('class', 'moz-smiley-' + strSml)

      const intElement = editor.createElementWithDefaults('span')
      if (!intElement) { return }

      // just for mailnews, because of the way it removes HTML
      const smileButMenu = document.getElementById('smileButtonMenu')
      if (smileButMenu.getAttribute('padwithspace')) { smileyCode = ' ' + smileyCode + ' ' }

      const txtElement = editor.document.createTextNode(smileyCode)
...
var nsAdvancedPropertiesCommand =
{
  ...
  doCommand: function (aCommand) {
    ...
    try {
      const element = GetCurrentEditor().getSelectedElement('')
      doAdvancedProperties(element)
    } catch (e) {}
  }
}
...
var nsEditLinkCommand =
{
  ...
  doCommand: function (aCommand) {
    try {
      const element = GetCurrentEditor().getSelectedElement('href')
      if (element) { editPage(element.href, window, false) }
    } catch (e) {}
    window.content.focus()
  }
}
...
var nsInsertOrEditTableCommand =
{
  ...
  doCommand: function (aCommand) {
    if (IsInTableCell()) { EditorTableCellProperties() } else { EditorInsertOrEditTable(true) }
  }
}
...
var nsDeleteTableRowCommand =
{
  ...
  doCommand: function (aCommand) {
    let rows = GetNumberOfContiguousSelectedRows()
...
var nsDeleteTableColumnCommand =
{
  ...
  doCommand: function (aCommand) {
    let columns = GetNumberOfContiguousSelectedColumns()
...
var nsJoinTableCellsCommand =
{
  isCommandEnabled: function (aCommand, dummy) {
    if (IsDocumentEditable() && IsEditingRenderedHTML()) {
      try {
        const editor = GetCurrentTableEditor()
        const tagNameObj = { value: '' }
        const countObj = { value: 0 }
        const cell = editor.getSelectedOrParentTableElement(tagNameObj, countObj)
        ...
        if (cell && (tagNameObj.value == 'td')) {
          ...
          if (!colSpan) { colSpan = Number(1) } else { colSpan = Number(colSpan) }

          const rowObj = { value: 0 }
          const colObj = { value: 0 }
    ...
    if (IsDocumentEditable() && IsEditingRenderedHTML()) {
      ...
      if (selection && !selection.isCollapsed) {
        ...
        if (GetParentTableCell(selection.focusNode) !=
             GetParentTableCell(selection.anchorNode)) { return false }
...
{
  const nodeName = firstNode.nodeName
};
