...
NS_IMETHODIMP
msiMrowBoundFenceCaret::GetNodeAndOffsetFromMouseEvent(nsIEditor *editor, nsIPresShell *presShell, 
  ...
  *offset = INVALID;
  ...
  if (NS_SUCCEEDED(res))
  {
    ...
    if (*node == nsnull)
    {
      ...
      res = msiMCaretBase::GetNodeAndOffsetFromMouseEvent(editor, presShell, flags,
                                                          mouseEvent, node, offset);
    }
    if (*node == nsnull)
    {
