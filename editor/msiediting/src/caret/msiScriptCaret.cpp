...
NS_IMETHODIMP
msiScriptCaret::GetNodeAndOffsetFromMouseEvent(nsIEditor *editor, nsIPresShell *presShell, 
                                               ...
                                               nsIDOMNode **node, PRUint32 *offset)
{
  ...
  *offset = INVALID;
  ...
  if (NS_SUCCEEDED(res))
  {
    ...
    else if ( middleLf <= eventPoint.x && eventPoint.x <= middleRt)
    {
      m_offset = 1;
    ...
    if (*node == nsnull)
    {
      NS_ASSERTION(PR_FALSE, "Script failed to set node and offset.");
