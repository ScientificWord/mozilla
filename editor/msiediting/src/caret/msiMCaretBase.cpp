...
NS_IMETHODIMP
msiMCaretBase::GetNodeAndOffsetFromMouseEvent(nsIEditor *editor, nsIPresShell *presShell, 
  ...
  *offset = INVALID;
  ...
  if (NS_SUCCEEDED(res))
  {
    ...
    if (passToKid && kid)
    {
      ...
      msiMCaretBase::GetNodeFromFrame(kid, child);
      NS_ASSERTION(child != m_mathmlNode, "Frame and child frame are associated to the same node");
      if (child && (child != m_mathmlNode))
      {
        nsCOMPtr<msiIMathMLCaret> mathCaret;
        msiUtils::GetMathMLCaretInterface(editor, child, 0, mathCaret);
