...
msiMtable::msiMtable(nsIDOMNode* mathmlNode, PRUint32 offset)
: msiMInsertionBase(mathmlNode, offset, MATHML_MTABLE)
{
}
NS_IMETHODIMP
msiMtable::InsertNode(nsIEditor * editor,
  ...
  if (editor && m_mathmlNode && node)
  {
    ...
    if (child)
    {
      ...
      if (NS_SUCCEEDED(res) && mathmlEditing)
      {
        ...
        NS_ASSERTION(mathmlType == MATHML_MTR || mathmlType == MATHML_MLABELEDTR, 
                      "Child of Mtable is not Mtr or Mlabeledtr");
#endif        
...
NS_IMETHODIMP
msiMtable::InsertMath(nsIEditor * editor,
                      nsISelection  * selection, 
                      PRBool isDisplay,
                      nsIDOMNode * left,    
                      nsIDOMNode * right,    
...
NS_IMETHODIMP 
msiMtable::Inquiry(nsIEditor * editor,
                   PRUint32 inquiryID, 
                   PRBool * result)
{
  nsresult res(NS_OK);
...
NS_IMETHODIMP
msiMtable::InsertNodes(nsIEditor * editor,
                       nsISelection * selection, 
                       nsIArray * nodeList,
