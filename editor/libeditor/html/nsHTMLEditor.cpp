...
NS_IMETHODIMP
nsHTMLEditor::RebuildDocumentFromSource(const nsAString& aSourceString, nsAString & _retval)
{
//  const nsCString xml=NS_LITERAL_CSTRING("text/xml");
  NS_DEFINE_CID( kDOMParserCID, NS_DOMPARSER_CID );
  ForceCompositionEnd();
...
//  nsCOMPtr<nsISelection>selection;
  ...
  nsCOMPtr<nsIDOMNode> htmlNode;
  nsCOMPtr<nsIDOMElement> newHtmlElement;
  nsCOMPtr<nsIDOMNode> newHtmlNode;
  ...
  if (!newHtmlNode) // no html node means an error message is in the doc object
  {
    ...
    nsCOMPtr<nsIDocumentEncoder> docEncoder (do_CreateInstance(formatType.get(), &res));
                               ...
                               PRBool *aSeenBR)
{
  ...
  *outIsEmptyNode = PR_TRUE;
  ...
  PRBool isListItemOrCell =
    (nsHTMLEditUtils::IsList(aNode, mtagListManager)
      || nsHTMLEditUtils::IsTable(aNode, mtagListManager));
...
/* AString returnAString (in AString thisone); */
NS_IMETHODIMP nsHTMLEditor::ReturnAString(const nsAString & thisone, nsAString & _retval)
{
    _retval = thisone;
    return NS_OK;
}
...
void InitCharMap()
{
  if (charMap == nsnull) {
    ...
    charMap->AppendString(NS_LITERAL_STRING("{\\~A}"));
    charMap->AppendString(NS_LITERAL_STRING("{\\\"A}"));
    charMap->AppendString(NS_LITERAL_STRING("{\\r A}"));
...
nsHTMLEditor::FilterCharsForLaTeX(PRBool isUnicode, const nsAString & orig, nsAString & _retval)
{
  ...
  for (; cur < end; ++cur) {
    ...
    if (*cur < 0xA0)
    {
      ...
      if (usingUTF) thisChar.Append(*cur);
      ...
      if (isUnicode) {
        ...
        switch ((int)(*cur)) {
          ...
          case 0x0218:
            thisChar.Append(NS_LITERAL_STRING("{\\textcommabelow S}")); break;
          case 0x0219:
            thisChar.Append(NS_LITERAL_STRING("{\\textcommabelow s}")); break;
          case 0x021A:
          ...
          case 0xFB02 :
            thisChar.Append(NS_LITERAL_STRING("fl")); break;  
          case 0xFB03 :
